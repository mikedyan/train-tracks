...
## Code Patterns
- LESSON-DAY72-A: Prune Week 3 Day 4 (Delight Polish) — the cheapest way to warm up an "empty state" inside an existing modal is a *single conditional render line* + a tiny CSS rule, not a whole `<div class="empty-state">...</div>` template with its own animation. Concretely: `if (earnedCount === 0) html += '<div class="sticker-empty-hint">Play to earn your first sticker! 🚂</div>';` inside the existing `renderStickerContent()`, sandwiched between the progress bar and the sticker grid. That's 1 JS line + 2 CSS lines (rule + night-mode) = 3 LOC minimum. Adding a parallel "complete" variant doubles that to 6 LOC but covers both the *0/12* and *12/12* states symmetrically. Mid-state (1–11) shows nothing — the sticker grid itself is the progress indicator. The pattern generalizes: any kid-magic empty-state polish should be a conditional inside the existing renderer, not a new component.
- LESSON-DAY72-B: Prune-week "delight polish" day is structurally a *spending* day inside the week's saved-LOC margin. Day 2 + Day 3 had cut −82 LOC against the hard rule, so Day 4 had a comfortable budget to add a kid-friendly nicety without breaking the "net negative" mandate. The framing matters: if Day 4 is approached as "we have +6 LOC to play with" rather than "every line is precious," the polish lands. End-of-week is still −76 LOC, comfortably beating Cycle 2's −36. Plan future prune weeks the same way — save aggressively Days 2–3 to free Day 4 for small joy.
- LESSON-DAY72-C: When verifying a state-conditional UI from Playwright, test all three branches in one `evaluate` call by mutating `stickerState.earned` directly and re-calling `renderStickerContent()`. Save the original state, run 0/12 → 1/12 → 12/12 probes, then restore + `saveStickerState()` to avoid polluting the user's localStorage. Three branches, one round-trip, plus an explicit cleanup line. Faster than UI-driving each state and more reliable than relying on screenshot diffing for tiny text changes.
...
## Day 67 — Harden Week 3 Day 4: Fix Everything (code-health pass)

- LESSON-DAY67-A: **The `function NAME(` duplicate-scan caught the only real dup, but it needed an empty bug queue to surface as a priority.** Through Days 1–63 the project had been busy enough that "1 duplicate function declaration" was noise — the two byte-identical `el(tag, attrs)` helpers inside `createTrainSVG` and `createCarSVG` lived for 60+ days because they each only existed in their parent's lexical scope and weren't *broken*. The lesson isn't "find dups faster" — it's that a quiet Harden Thursday is the right moment for cosmetic refactors. Save risky structural changes for low-traffic days when the bug queue is empty, console is clean, and a regression has nowhere to hide.

- LESSON-DAY67-B: **`const el = svgEl;` is the safest "rename without rewriting call sites" pattern.** When hoisting a helper that has 30+ call sites inside two functions, the temptation is to find-and-replace `el(` → `svgEl(`. Don't. Aliasing the hoisted symbol back to the original local name preserves every existing call site untouched and reduces the diff to the function's declaration block only. Net result is the same memory footprint (helper hoisted once, aliased twice) but the diff stays tiny and reviewable. This is especially important for SVG-builder functions where one missed call site can produce a runtime crash and not a parse error.

- LESSON-DAY67-C: **Dynamic class-name construction is the #1 false-positive source in dead-CSS scans.** The naive sweep flagged 13 classes; after grepping for the *prefix* (`biome-`, `animal-react-`, `difficulty-`), 11 of the 13 turned out to be dynamically composed via template literals or `'class-' + variable` concatenation. Only 2 (`puzzle-card-complete` and `.station-passenger.arriving`) were truly dead. Build the sweep with two passes: (1) flag candidate dead classes by name absence; (2) for each candidate, *also* grep for its prefix and any string-concatenation patterns that could synthesize it. A class is only dead if **both** passes agree.

- LESSON-DAY67-D: **An orphaned `@keyframes` always travels with at least one orphaned rule.** The `passenger-arrive` keyframes were dead because the only rule referencing them (`.station-passenger.arriving { animation: passenger-arrive ... }`) was itself never applied. When removing a dead selector that has an `animation:` property, *always* check whether its referenced keyframes are used elsewhere — if not, remove them too. Searching by the keyframe name (`passenger-arrive`) instead of the class name (`arriving`) is the faster path to spot the orphan pair.

- LESSON-DAY67-E: **Module-scope SVG_NS replaces N copies of the same string constant.** Before today, `'http://www.w3.org/2000/svg'` appeared as a `const ns = ...` inside multiple SVG-builder functions (train, car, and possibly others). Hoisting `SVG_NS` to module scope means future SVG-builder additions can skip re-declaring it — they just reference `SVG_NS` (and `svgEl(...)` if appropriate). For the file size discipline this matters too: each future builder saves one line versus the old pattern.

## Day 75 — Cycle 4 Build Day 2: Station Arrival Signal 🚦

- LESSON-DAY75-A: **BFS-on-rail beats Manhattan distance for "incoming train" detection.** First instinct was `Math.abs(stationRow - trainRow) + Math.abs(stationCol - trainCol) <= 2`. That would have lit a yellow signal for a train sitting on a parallel siding 2 cells north of the station — a kid would expect green there because the train *can't reach* the station along the rails. Doing a depth-3 BFS along connected pieces (using the existing `getConnections` + `neighborConnected` predicates) ensures the signal only triggers when a train can physically arrive in N moves. Cost is trivial (≤10 stations × ≤25 nodes/BFS @ ~8 Hz) — the throttle (`now - lastUpdate < 120ms`) keeps even a 60 fps render-loop cheap.

- LESSON-DAY75-B: **`data-state` attribute > 3-class swap for tri-state visual indicators.** Tempting alternative was `el.classList.add('signal-red'); classList.remove('signal-yellow', 'signal-green')`. With `el.dataset.state = newState`, the state machine is single-attribute (no risk of two states active simultaneously) and CSS lights up the right dot via attribute selectors (`.station-signal[data-state="red"] .signal-dot.signal-red { ... }`). Diff is also smaller: 3 selector lines vs 9 classList ops in JS.

- LESSON-DAY75-C: **The throttle check happens *before* the early-return on empty list.** Original draft had `if (!stationSignalState.els.length) return;` first, then the time check. After thinking about it: the empty-list case will run every frame anyway (zero stations = zero work either way), so order doesn't matter for perf. But putting the throttle gate first is cleaner because it expresses "only do work every 120ms" as the universal rule, and the inner loop is then trivially correct. Tiny readability win.

- LESSON-DAY75-D: **`box-shadow` with the dot color = "lit lamp" effect, near-free.** Tried first with `background` alone — the dots looked flat. Added `box-shadow: 0 0 4px #ff4136` (matching glow color per state) and they instantly read as "active light bulbs". One CSS line per state, no JS, no @keyframes needed for the glow itself. Pulse animation is layered on top via `animation: signal-pulse 0.6s ease-in-out infinite` only for red (urgent) and yellow (warning); green stays steady because "no incoming train" doesn't need to attract attention. Tiered animation timing matches real-world stoplight psychology: red blinks fastest, yellow moderate, green is calm.

- LESSON-DAY75-E: **Per-station `position: relative` was already guaranteed by `.cell { position: relative }`** (line 378). New visual overlays inside the cell can use `position: absolute; top: 4px; right: 4px` directly without scaffolding. Took 30 seconds to verify this once; saved a defensive wrapper div in the JS. **Always check the host element's existing CSS before adding a positioning wrapper** — same principle as Day 60's `cell.querySelectorAll('.station-passenger')` overlay approach.
